...
function ChildCard({ title, accent, child, setChild }) {
    const update = (patch) => setChild({ ...child, ...patch });
    const setDay = (i, v) => {
        const s = [...child.schedule]; s[i] = v; update({ schedule: s });
    };
    return (
        <div className={`bg-white rounded-2xl p-5 shadow-sm border-t-4 ${accent}`}>
            <div className="flex items-baseline justify-between mb-3">
                <h2 className="text-xl font-semibold text-slate-800">{title}</h2>
                <span className="text-xs text-slate-400">Busy Bees Tunbridge Wells</span>
            </div>

            <div className="grid grid-cols-2 gap-3 mb-4">
                <label className="text-sm">
                    <span className="block text-slate-500 mb-1">Age bracket</span>
                ...
                <label className="text-sm">
                    <span className="block text-slate-500 mb-1">Funded hours</span>
            ...
            <div className="space-y-2 mb-3">
                <div className="text-sm font-medium text-slate-600">Attendance</div>
                {DAYS.map((d, i) => (
                    <div key={d} className="flex items-center gap-2">
                        <div className="w-24 text-sm text-slate-600">{d}</div>
                        <select value={child.schedule[i]} onChange={e => setDay(i, e.target.value)}
                                className="flex-1 rounded-lg border border-slate-200 px-2 py-1.5 bg-white text-sm">
...
function MilPanel({ mil, setMil }) {
    return (
        <div className="bg-white rounded-2xl p-5 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-800 mb-1">Mother-in-law contribution</h2>
            ...
            <div className="grid grid-cols-5 gap-2">
                {DAYS.map((d, i) => (
                    <label key={d} className="text-center">
                        <span className="block text-xs text-slate-500 mb-1">{d.slice(0, 3)}</span>
                        <select value={mil[i]}
                                onChange={e => {
                                    const n = [...mil]; n[i] = Number(e.target.value); setMil(n);
                                }}
                                className="w-full rounded-lg border border-slate-200 px-2 py-1.5 bg-white text-sm">
...
function AdHocPanel({ adhoc, addAdHoc, removeAdHoc, monthAdhocs, monthLabel }) {
    ...
    return (
        <div className="bg-white rounded-2xl p-5 shadow-sm">
            <h2 className="text-lg font-semibold text-slate-800 mb-1">Ad-hoc days</h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mb-3">
                ...
                <label className="text-xs">
                    <span className="block text-slate-500 mb-1">Child</span>
                ...
                <label className="text-xs">
                    <span className="block text-slate-500 mb-1">Session</span>
            ...
            <button
                ...
                className="w-full text-sm font-medium bg-amber-500 hover:bg-amber-600 text-white rounded-lg px-3 py-2 mb-3">
                + Add ad-hoc day
            </button>

            {adhoc.length === 0 ? (
                <p className="text-xs text-slate-400 italic">No ad-hoc days added.</p>
            ) : (
                <div>
                    <div className="text-xs font-medium text-slate-600 mb-1">
                        All ad-hoc days ({adhoc.length}){monthAdhocs.length !== adhoc.length &&
                            <span className="text-slate-400 font-normal"> · {monthAdhocs.length} in {monthLabel}</span>}
                    </div>
                    <ul className="text-xs space-y-1 max-h-40 overflow-y-auto">
                        {adhoc.slice().sort((a, b) => a.date.localeCompare(b.date)).map(a => {
                            ...
                            return (
                                <li key={a.id}
                                    className={`flex items-center justify-between rounded-lg px-2 py-1.5 ${inMonth ? 'bg-amber-50' : 'bg-slate-50'}`}>
                                    <span className="flex-1">
                                        <span className="font-medium">{a.date}</span>
                                        <span className="text-slate-500"> · {a.child === 'ellis' ? 'Ellis' : 'Gaspard'} · {a.type === 'fullDay' ? 'Full Day' : a.type === 'morning' ? 'Morning' : 'Afternoon'} · {a.ageBracket}</span>
                                    </span>
...
const NurseryPage = () => {
    ...
    useEffect(() => {
        ...
        saveTimeout.current = setTimeout(() => {
            apiService.updateNurserySettings({
                ellis, gaspard, mil, taxFree, fullWeekModel, showBreakdown, adhoc,
            }).catch(err => console.error('Nursery settings save failed', err));
        }, 500);
        return () => { if (saveTimeout.current) clearTimeout(saveTimeout.current); };
    }, [loaded, ellis, gaspard, mil, taxFree, fullWeekModel, showBreakdown, adhoc]);
    ...
    const calc = useMemo(() => {
        ...
        const ellisStretched   = weeklyStretched(ellis.schedule, ellis.ageBracket, ellis.scheme, fullWeekModel);
        const ellisStandard    = weeklyStandard(ellis.schedule, ellis.ageBracket);
        const gaspardStretched = weeklyStretched(gaspard.schedule, gaspard.ageBracket, gaspard.scheme, fullWeekModel);
        const gaspardStandard  = weeklyStandard(gaspard.schedule, gaspard.ageBracket);

        const eSib = ellis.siblingDiscount ? 0.90 : 1.00;
        const gSib = gaspard.siblingDiscount ? 0.90 : 1.00;
        const tfMult = taxFree ? 0.80 : 1.00;
        ...
        const monthlyDaily = [0, 1, 2, 3, 4].map(i => {
            ...
            const milGrossPay   = combined * (mil[i] / 100);
            const milPay        = milGrossPay * tfMult;
            const parentPay     = (combined - milGrossPay) * tfMult;
            return {
                eFundedType: ellis.schedule[i],
                gFundedType: gaspard.schedule[i],
        ...
        const milSummary = [0, 1, 2, 3, 4].map(i => {
            const percent = mil[i];
            const eType = ellis.schedule[i];
            const gType = gaspard.schedule[i];
    ...
    }, [ellis, gaspard, mil, taxFree, fullWeekModel, monthOffset, startDate, adhoc]);

    return (
        <div className="-m-4 md:-m-4 p-4 md:p-8 bg-amber-50 rounded-2xl">
            <header className="mb-6 flex items-start justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">Nursery Cost Calculator</h1>
                    <p className="text-slate-500">Busy Bees Tunbridge Wells · fees effective 1 January 2026</p>
                    <p className="text-xs text-slate-400 mt-1">Your settings &amp; ad-hoc days are saved to your account.</p>
                </div>
                <button
                    onClick={() => {
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
                <ChildCard title="Ellis"   accent="border-amber-400" child={ellis}   setChild={setEllis} />
                <ChildCard title="Gaspard" accent="border-sky-400"   child={gaspard} setChild={setGaspard} />
            </div>

            <div className="grid md:grid-cols-2 gap-4 mb-4">
                <MilPanel mil={mil} setMil={setMil} />
                <div className="bg-white rounded-2xl p-5 shadow-sm">
                    <h2 className="text-lg font-semibold text-slate-800 mb-3">Billing model & discounts</h2>
                    <div className="space-y-2">
                        <Toggle checked={fullWeekModel} onChange={setFullWeekModel}
                                label="Full Week model (£356/wk → £7.12/hr on non-funded hours)" />
                        <Toggle checked={taxFree} onChange={setTaxFree}
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm mb-4 flex items-center justify-between gap-3">
                <button
                    onClick={() => setMonthOffset(o => o - 1)}
                    className="px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium flex items-center gap-1">
                    <ChevronLeft className="h-4 w-4" /> Previous
                </button>
                <div className="text-center flex-1">
                    <div className="text-xs uppercase tracking-wide text-slate-400">Showing</div>
                    <div className="text-xl font-semibold text-slate-800">{calc.monthLabel}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div className="bg-gradient-to-br from-amber-400 to-amber-500 text-white rounded-2xl p-5 shadow">
                ...
                <div className="bg-gradient-to-br from-rose-400 to-rose-500 text-white rounded-2xl p-5 shadow">
                    ...
                    <div className="text-xs text-rose-50 mt-1">
                        {taxFree ? 'her share × 80% (tax-free)' : 'her share of the bill'}
                    </div>
                </div>
                <div className="bg-gradient-to-br from-indigo-400 to-indigo-500 text-white rounded-2xl p-5 shadow">
            ...
            {calc.milSummary.length > 0 && (
                <div className="bg-white rounded-2xl p-5 shadow-sm mb-4 border-l-4 border-rose-300">
                    <div className="flex items-baseline justify-between mb-2">
                        <h2 className="text-lg font-semibold text-slate-800">What your mother-in-law should transfer</h2>
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                        ...
                        {taxFree && ' Her transfer is reduced by 20% because tax-free childcare tops up her share too.'}
                    </p>
            ...
            {showBreakdown && (
                <div className="bg-white rounded-2xl p-5 shadow-sm mb-4">
                    <h2 className="text-lg font-semibold text-slate-800 mb-1">Breakdown for {calc.monthLabel}</h2>
                    <p className="text-xs text-slate-500 mb-3">
                            ...
                            <tbody>
                                {DAYS.map((d, i) => {
                                    ...
                                    return (
                                        <tr key={d} className="border-b last:border-none align-top">
                                            ...
                                            <td className="py-2 pr-3 text-right num text-slate-700">{countLabel}</td>
                                            <td className="py-2 pr-3 text-slate-600 text-xs leading-tight">
                                                {eSession && <div>{eSession}</div>}
                                                {gSession && <div>{gSession}</div>}
                                                {!eSession && !gSession && <div className="text-slate-400">–</div>}
                                            </td>
                                ...
                                {calc.monthAdhocs.map(a => (
                                    <tr key={a.id} className="border-b last:border-none bg-amber-50/50 align-top">
                                        ...
                                        <td className="py-2 pr-3 text-right num text-slate-700">1</td>
                                        <td className="py-2 pr-3 text-slate-600 text-xs leading-tight">
                    ...
                    <div className="mt-5">
                        <div className="bg-white border border-slate-200 rounded-xl p-4">
                            <div className="text-sm font-semibold text-slate-700 mb-1">Invoice from nursery</div>
                            <p className="text-xs text-slate-500 mb-3">Invoiced = nursery's per-child total (after sibling discount). MIL covers = what she actually transfers per child{taxFree ? ' (her share × 80% so the TFC top-up matches)' : ''}. Transfer to TFC = invoice × 80% (the government tops up the rest).</p>
                            {(() => {
                                const milMult = taxFree ? 0.80 : 1.00;
            ...
            <footer className="text-xs text-slate-400 mt-6 space-y-1">
                <p>
