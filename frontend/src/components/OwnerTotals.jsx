...
export const SharedCard = ({ billsPotTotal, groceriesPotTotal, sharedIncome, sharedExpenses, extraTotal = 0, sharedSavings = 0, totalContributions }) => {
    const remaining = sharedIncome + totalContributions - sharedExpenses - sharedSavings;
    return (
        <div className="p-5 bg-white rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-shadow flex flex-col h-full">
            ...
            <div className="space-y-3 text-sm grow">
                ...
                {sharedSavings > 0 && (
                    <div className="flex justify-between items-center"><span className="flex items-center text-gray-600"><PiggyBank className="mr-2 h-4 w-4 text-sky-500" />Savings</span> <span className="font-semibold text-sky-600">- £{sharedSavings.toFixed(2)}</span></div>
                )}
                {extraTotal > 0 && (
    ...
    return (
        <div className="p-5 bg-white rounded-xl shadow-md border border-gray-100 hover:shadow-lg transition-shadow flex flex-col h-full">
            ...
            <div className="space-y-3 text-sm grow">
                ...
                <div className="flex justify-between items-center"><span className="flex items-center text-gray-600"><TrendingDown className="mr-2 h-4 w-4 text-red-400" />Transfer to joint account</span> <span className="font-semibold text-red-500">- £{share.toFixed(2)}</span></div>
